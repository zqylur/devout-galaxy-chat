...
class Chat extends Component {
  // The state is initialized in the constructor and the functions below are bound with 'this'.
    ...
    componentWillMount() {
        ...
        this.pusher = new Pusher('373570', {
            authEndpoint: '/pusher/auth',
            cluster: 'us2',
            encrypted: true
        });
        // Subscribe to the 'private-reactchat' channel
        this.chatRoom = this.pusher.subscribe('private-devout-galaxy-chat');
    }
    ...
    sendMessage(event) {
        ...
        if (this.state.value !== '') {
            ...
            // var messages = this.state.messages.concat(this.state.value);
            this.setState({value: ''})
        }
        else {
    ...
    render() {
        ...
        const message = messages.map((item, ind) => {
            return (
                <Grid key={ind}>
                    {message}
